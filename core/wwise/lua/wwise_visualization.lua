...
local function render_soundscape_unit(lines, lines_noz, unit)
	...
	if Wwise.position_type(event_name) == Wwise.WWISE_3D_SOUND then
		...
		elseif shape == "sphere" then
			--local shape_radius = math.max(Vector3.x(scale), Vector3.y(scale), Vector3.z(scale))
		...
		elseif shape == "box" then
			-- remove scale from matrix pose as it is multiplied by the extents in add_box and we need the extents
