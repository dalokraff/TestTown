...
function M.wwise_set_state(t)
	...
	if (not group) or (not state) then return end
...
function M.wwise_get_playing_elapsed(t)
	...
	if not elapsed_in_ms then elapsed_in_ms = 0 end
...
function M.wwise_remove_soundscape_source(t)
	...
	if id == -1 then -- This can happen if the Add node fails or if the output is referenced without ever running Add
		return
	end
...
-- If Wwise is not available, the callback functions do nothing.
if not Wwise then
